	...
	else
	{
		...
		if((isset($_SESSION["saved_locations"])))
		{
			...
			else if($_SERVER["REQUEST_METHOD"] == "POST")
			{
				...
				$relationship_sought = (empty($_POST["relationship_sought"]) )?"":sumCheckBox($_POST["relationship_sought"]);
				if($relationship_sought != "")
				{
					setcookie("relationship_sought",$relationship_sought, time() + COOKIE_EXPIRE_DATE);
					$sql .= buildOrCondition("relationship_sought", $relationship_sought);
				}
				
				$body_type = (empty($_POST["body_type"]) )?"":sumCheckBox($_POST["body_type"]);
				if($body_type != "")
				{
				...
				$horoscope = (empty($_POST["horoscope"]) )?"":sumCheckBox($_POST["horoscope"]);
				if($horoscope != "")
				{
				...
				$ethnicity = (empty($_POST["ethnicity"]) )?"":sumCheckBox($_POST["ethnicity"]);
				if($ethnicity != "")
				{
				...
				$religion = (empty($_POST["religion"]) )?"":sumCheckBox($_POST["religion"]);
				if($religion != "")
				{
				...
				$children = (empty($_POST["children"]) )?"":sumCheckBox($_POST["children"]);
				if($children != "")
				{
				...
				$smoking = (empty($_POST["smoking"]) )?"":sumCheckBox($_POST["smoking"]);
				if($smoking != "")
				{
				...
				$education = (empty($_POST["education"]) )?"":sumCheckBox($_POST["education"]);
				if($education != "")
				{
				...
				$sql .= " AND users.user_id = profiles.user_id AND users.user_type <> 'd'
						ORDER BY users.last_access DESC LIMIT " . RESULT_LIMIT;
				echo $sql;
				...
				if($error == "")
				{
					...
					$_SESSION['search_results']=$array_of_found_users;
...
<table cellspacing="15px" cellpadding="12px" style="text-align:left; margin: 0px auto 0px auto" width="100%">
	...
	<tr>
		<p align="center">
			Locations are set to <?php echo $_COOKIE["city_name_list"]; ?> <br/>
